...
bool inicializarESP(uint32_t timeoutMs){
    ...
    // 1) Enviar "AT" periÃ³dicamente hasta recibir "OK"
...
bool resetearCredencialesESP(void){
    ...
    if (n > 0){
        ...
        // Buscar "OK" seguido de CRLF (mÃ¡s robusto que solo OK)
        // La respuesta tÃ­pica es: ...AT+RESTORE\r\n\r\nOK\r\n\r\nready\r\n
        bool foundOK = false;
        ...
        if(foundOK){
            ...
            printf("\r\n[ESP][RESET] El ESP se reiniciarÃ¡...\r\n");
            delay(3000); // Esperar a que el ESP se reinicie completamente
            
            // Re-inicializar comunicaciÃ³n AT
            printf("\r\n[ESP][RESET] Reinicializando comunicaciÃ³n...\r\n");
            delay(1000);
...
size_t enviarComandoAT(const char *cmd, char *respuesta, size_t maxLen, uint32_t timeoutMs){
    ...
    while ((tickRead() - start) < timeoutMs){
        if (uartReadByte(ESP_UART, &byte)){
            ...
            // Si el buffer estÃ¡ al lÃ­mite, salimos conservando espacio para null-terminator
            if (pos >= (maxLen - 1)){
                break;
            }
